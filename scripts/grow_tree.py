...
def make_config(args) -> TreeConfig:
    ...
    if args.terrain and args.preset == "pergola" and args.terrain_seed is None:
        import random
...
def parse_args():
    ...
    t.add_argument("--terrain-amplitude", type=float, default=0.05,
                   help="height range [m] above a 4 mm ground offset (default 0.05); "
                        "rougher settings are not validated for robot locomotion")
    t.add_argument("--terrain-wavelength", type=float, default=1.8,
                   help="dominant bump spacing [m] (default 1.8)")
    t.add_argument("--terrain-extent", type=float, default=14.,
                   help="ground half-extent [m] (default 14)")
    t.add_argument("--terrain-seed", type=int, default=None,
                   help="fixed terrain seed for replay; omitted picks fresh random kiwi "
                        "terrain each launch (apple uses the scene --seed)")
