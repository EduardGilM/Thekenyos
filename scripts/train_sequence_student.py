"""Sensor-only student: hand RGB-D plus proprioception, distilled from the privileged sequence teacher.

The student never receives simulator state. The teacher's privileged actions supervise it
(DAgger-style: the student acts, the teacher labels), on top of the same PPO objective.
"""
import argparse
import json
import os
...
STUDENT_SCHEMA = 'fast-harvest-sequence-student-rgbd-r84/v1'
...
def evaluate(runtime, policy, gait, args, level, seed):
    ...
    with torch.random.fork_rng(devices=[torch.cuda.current_device()]):
        ...
        collector = HarvestCollector(runtime, role='student', reward_profile=SEQUENCE_PROFILE,
...
def run(args):
    ...
    config.update(role='student', reward_profile=SEQUENCE_PROFILE, reward_gamma=SEQUENCE_GAMMA,
        ...
        actor_inputs='hand-camera RGB-D (25 Hz) and R84 proprioception only; no simulator state, no episode history',
        supervision='PPO on the sequence reward plus MSE to the privileged teacher action on the student\'s own states',
    ...
    try:
        ...
        runtimes = [FastRuntime(scene, worlds=worlds, camera='hand_camera', resolution=tuple(args.resolution), task_profile=SEQUENCE_PROFILE,
        ...
        teacher = build_privileged_policy(sequence=True).cuda().eval()
        for parameter in teacher.parameters():
            parameter.requires_grad_(False)
        teacher_meta = load_checkpoint(args.teacher_checkpoint, {'teacher': teacher}, None,
            expected_meta={'role': 'teacher', 'model_sha256': runtime.manifest['model_sha256']})['meta']
        config['teacher_checkpoint'] = str(args.teacher_checkpoint)
        config['teacher_schema'] = teacher_meta.get('schema')
        curriculum = Curriculum(level=args.start_level, max_level=args.max_level)
        optimizer = torch.optim.Adam(policy.parameters(), lr=args.learning_rate)
        if args.resume_from:
            saved = load_checkpoint(args.resume_from, {'student': policy}, {'student': optimizer},
                expected_meta={'schema': STUDENT_SCHEMA, 'model_sha256': runtime.manifest['model_sha256']})
        ...
        collector = HarvestCollector(runtime, role='student', teacher_policy=teacher, reward_profile=SEQUENCE_PROFILE,
        ...
        def checkpoint(render_objective=None):
            ...
            save_checkpoint(path, {'student': policy}, {'student': optimizer},
                ...
                dict(schema=STUDENT_SCHEMA, role='student', camera='hand_camera', camera_profile=runtime.manifest['cameras'], config=config, update=index,
        ...
        (args.output/'active-process.json').write_text(json.dumps(dict(pid=os.getpid(), status='running', phase='sequence-student')))
        ...
        while time.monotonic()-started < args.train_seconds:
            ...
            for epoch in range(3):
                result = update(policy, optimizer, rows, bootstrap, args.minibatch_worlds,
                    ...
                    check_replay=epoch==0, minibatch_updates=True, target_kl=.02, teacher_coef=args.teacher_distill_weight)
...
def main():
    ...
    p.add_argument('--start-level', type=int, default=3, help='Objective the student starts on (the teacher already masters the prefix)')
    p.add_argument('--resolution', type=int, nargs=2, default=(64, 48))
    ...
    p.add_argument('--fruit-sector-deg', type=float, default=70.)
    ...
    if not (1 <= a.worlds <= 4096 and 32 <= a.eval_worlds <= 256 and 2 <= a.steps <= 256
            ...
            and 1 <= a.stall_seconds < a.max_episode_seconds <= 60 and 0 < a.arm_speed_rad_s <= 2.5 and -1.5708 <= a.initial_jaw_rad <= 0 and 0 < a.jaw_rate_rad_s <= 5 and 0 <= a.fruit_damping <= .05 and 1 <= a.start_level <= a.max_level <= 5 and 0 <= a.teacher_distill_weight <= 10
