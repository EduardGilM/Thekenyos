...
def main():
    ...
    if a.replay:
        ...
        with torch.cuda.stream(stream), wp.ScopedStream(wp.stream_from_torch(stream)):
            ...
            graph_profile=reward_profile in ('graph-harvest/v1','graph-harvest/v2','graph-harvest/v3','graph-harvest/v4')
            ...
            if reward_profile in ('graph-harvest/v3','graph-harvest/v4'): metadata['success']=bool(progress.graph_success[0])
            if graph_profile:
