...
def main():
    ...
    if a.replay:
        ...
        with torch.cuda.stream(stream), wp.ScopedStream(wp.stream_from_torch(stream)):
            checkpoint_meta = json.loads(Path(str(a.checkpoint)+'.json').read_text())
            from treesim.kiwi_rl.reward_graph import GRAPH_PROFILES, SEQUENCE_PROFILE
            sequence = checkpoint_meta.get('config', {}).get('reward_profile') == SEQUENCE_PROFILE
            if a.role == 'teacher':
                from treesim.kiwi_rl.fast_teacher import build_privileged_policy, privileged_observation
                policy = build_privileged_policy(sequence=sequence).cuda().eval()
            else:
            ...
            graph_profile=reward_profile in GRAPH_PROFILES
            rt = FastRuntime(a.scene, worlds=1, camera='hand_camera',arm_speed_rad_s=arm_speed,
                             ...
                             absolute_jaw=bool(saved['meta'].get('config', {}).get('absolute_jaw', False)),
                             initial_jaw_rad=saved['meta'].get('config', {}).get('initial_jaw_rad'),
                             jaw_rate_rad_s=saved['meta'].get('config', {}).get('jaw_rate_rad_s', 1.),
                             fruit_damping=saved['meta'].get('config', {}).get('fruit_damping', 0.),
                             fruit_jitter_m=tuple(saved['meta'].get('config', {}).get('fruit_jitter_m', (0., 0., 0.))),
                             fruit_reach_fraction=tuple(saved['meta'].get('config', {}).get('fruit_reach_fraction', (0., 0.))),
                             fruit_sector_deg=saved['meta'].get('config', {}).get('fruit_sector_deg', 70.))
            ...
            if sequence:
                ...
                progress_kwargs['curriculum_stage'] = saved['meta']['render_objective']
            progress=progress_type(signals(rt), reward_profile=reward_profile, guidance=0., **progress_kwargs,
            ...
            with torch.no_grad():
                for i in range(a.steps):
                    ...
                    obs = rt.observe()
                    inputs = privileged_observation(rt, progress=progress) if a.role == 'teacher' else rgbd
            ...
            if reward_profile in ('graph-harvest/v3','graph-harvest/v4','graph-harvest/v5') or sequence: metadata['success']=bool(progress.graph_success[0])
            if sequence:
                metadata.update(prefix_success=bool(progress.prefix_success[0]),
                    objective=progress.curriculum_stage, completed={k:bool(v[0]) for k,v in progress.graph_completed.items()})
            if graph_profile:
