...
def _easy_airdrop(mask: wp.array(dtype=wp.uint8), reset_mode: wp.array(dtype=int),
    ...
    Not a liner teleport. Independent of TCP so an outside-crate arm start
    does not leave the fruit beside the crate or put fingers in the hole.
    """
    world = wp.tid()
    if mask[world] == 0 or reset_mode[world] != 1:
        return
...
class FastRuntime:
    ...
    def enable_easy(self, enabled=True, *, shaping_coef=None, open_xy_m=None):
        """Train-only facilitation: outside-crate arm start + stronger shaping.

        Does not weld fruit, spawn the arm inside the crate, or write fruit
        into the liner. Fruit is airdropped over the opening so the outside-crate
        arm start does not drop it onto the chassis. Eval still sets
    ...
    def privileged_deposit_action(self):
        ...
        After an easy airdrop the fruit is already over the opening, so the
        teacher must not drive the gripper into the hole.
        """
        import torch
