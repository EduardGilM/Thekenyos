...
class FastTrainerCLITest(unittest.TestCase):
    def test_single_cli_defaults_to_deposit_pixels(self):
        ...
        self.assertIn('carry_waypoint_mean', inspect.getsource(train_fast.run))
    ...
    def test_easy_cli_fills_teacher_mix_but_keeps_explicit_zero(self):
        ...
        self.assertIn('set_carry_line_enabled', run_src)
        self.assertIn('set_carry_line_enabled(not demo_phase)', run_src)
        self.assertIn('set_carry_line_enabled(False)', inspect.getsource(train_fast.evaluate_mission))
        ...
        self.assertIn('_set_shape_offsets', runtime_src)
        self.assertIn('potential_ref = 3', runtime_src)
    ...
    def test_apply_stage_mixes_harvest_deposit_starts(self):
        ...
        sys.path.insert(0, str(Path(__file__).resolve().parents[1] / 'scripts'))
        import train_fast
        from treesim.kiwi_rl.curriculum import RESET_DEPOSIT, RESET_PREGRASP, stage_named
