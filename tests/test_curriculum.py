...
class CurriculumTest(unittest.TestCase):
    ...
    def test_easy_preset_does_not_change_gates_or_weld(self):
        ...
        self.assertAlmostEqual(float(over[2]), float(CENTER[2] + SIZE[2] + 0.28))
        ...
        for pose in sampled:
            ...
            self.assertGreaterEqual(float(pose[2]), float(hi[2] + 0.28) - 1e-9)
            self.assertLessEqual(float(pose[2]), float(hi[2] + 0.28 + 0.04) + 1e-9)
