...
class CurriculumTest(unittest.TestCase):
    ...
    def test_easy_preset_does_not_change_gates_or_weld(self):
        ...
        self.assertEqual(preset['teacher_mix'], 0.0)
        ...
        self.assertEqual(preset['start_margin_m'], 0.22)
        self.assertEqual(preset['start_x_span_m'], 0.12)
        ...
        self.assertAlmostEqual(easy_teacher_mix(10, anneal_after=20), 1.0)
        self.assertAlmostEqual(easy_teacher_mix(20, anneal_after=20), 1.0)
        self.assertAlmostEqual(easy_teacher_mix(50, anneal_after=20), 0.5)
        self.assertAlmostEqual(easy_teacher_mix(80, anneal_after=20), 0.0)
        with self.assertRaises(ValueError):
        ...
        for pose in sampled:
            ...
            self.assertGreaterEqual(float(pose[0]), float(hi[0] + 0.22) - 1e-9)
            self.assertLessEqual(float(pose[0]), float(hi[0] + 0.22 + 0.12) + 1e-9)
