...
`--easy` is an experimental student-side shortcut on top of that chain, not a second curriculum. Deposit resets solve a chassis-frame hover above the open basket rim, then place a **free** fruit at the new TCP with the jaw closed (same physical stem-release rule; no weld and no liner teleport). Training mixes a privileged joint-space teacher toward that hover and opens the jaw when fruit XY is over the basket (`teacher_mix=0.4` unless overridden). Potential shaping uses a graph-safe coefficient of 5 instead of 2. Evaluation still has `guidance_weight=0` and teacher mix 0; promotion gates do not change. Log `basket_distance_m` / `basket_xy_m` for deposit, because TCP–fruit distance is near zero after a grasp-style reset. A speedrun that starts deposit from the default hanging TCP often drops the fruit immediately; `--easy` is the intended student-side shortcut for that stage. CPU progress clips read `config.easy` and use the same hover reset. A successful easy deposit is not a field harvest.
