...
`--easy` is an experimental student-side shortcut on top of that chain, not a second curriculum. Deposit resets spawn the arm **outside** the crate with a 40 cm robot-side margin and 28 cm rim clearance so the wrist is not spawned through the liner, then place a **free** fruit at the TCP with the jaw closed. The start recedes toward the home-side carry over 200 PPO updates (`easy_far_frac` 0→1) so the policy has to move the fruit to the basket. The privileged teacher aims at a high drop pose over the rim and holds still / opens the jaw when fruit XY is over the basket (`teacher_mix` starts at 0.4 unless overridden, then linearly drops to 0 over 60 PPO updates so the student must deposit before the first teacher-off eval). Fast-scene load moves `basket_floor` / `basket_liner` onto a massless `basket_shell` child of the chassis and adds explicit arm-crate contact pairs so Spot's chassis–arm excludes and `filterparent` no longer let the gripper occupy the crate; visual vents stay on the chassis and basket mass stays on the chassis inertial. Potential shaping uses a graph-safe coefficient of 5 instead of 2. One-world nonfinite/overflow faults reset that world instead of killing the 3072-world job; `latest.pt` is overwritten every update so a crash does not throw away the previous weights. Evaluation still has `guidance_weight=0` and teacher mix 0; promotion gates do not change. Log `basket_distance_m` / `basket_xy_m` / `easy_far_frac` / `teacher_mix` for deposit. Jaw-overload failure applies to grasp/detach, not to DEPOSIT_ONLY fruit already falling past the gripper. A successful easy deposit is not a field harvest. Spawn-in-volume and liner teleport stay disallowed.
