...
`--easy` is an experimental student-side shortcut on top of that chain, not a second curriculum. Deposit resets solve a chassis-frame hover above the open basket rim, then place a **free** fruit over the basket opening (world XY of the chassis-frame basket centre) and 10 cm below the new TCP Z with the jaw open so gravity can start the drop (same physical stem-release rule; no weld and no liner teleport). The XY recentre removes hover-IK horizontal error so the fruit is not spawned beside the rim. Training mixes a privileged joint-space teacher toward that hover and keeps the jaw open when fruit XY is over the basket (`teacher_mix=0.4` unless overridden). Potential shaping uses a graph-safe coefficient of 5 instead of 2. Evaluation still has `guidance_weight=0` and teacher mix 0; promotion gates do not change. Log `basket_distance_m` / `basket_xy_m` for deposit, because TCP–fruit distance is the wrong metric after a grasp-style reset. Jaw-overload failure applies to grasp/detach, not to DEPOSIT_ONLY fruit already falling past the gripper. A successful easy deposit is not a field harvest.
