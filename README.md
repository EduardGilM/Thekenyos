...
`--easy` is an experimental student-side shortcut on top of that chain, not a second curriculum. Deposit resets spawn the arm **outside** the crate (robot-side of the front wall, above the rim) so the gripper does not occupy the opening, then place a **free** fruit over the basket centre 6 cm above the rim with the jaw open (gravity starts the drop; no weld and no liner teleport). The arm start recedes toward the home-side carry over 400 PPO updates (`easy_far_frac` 0→1). The privileged teacher holds still when fruit XY is already over the basket and otherwise aims at a high drop pose over the rim (`hover_clearance_m=0.28`, `teacher_mix=0.4` unless overridden). Fast-scene load moves `basket_floor` / `basket_liner` onto a massless `basket_shell` child of the chassis so Spot's chassis–arm contact excludes no longer let the gripper occupy the crate; visual vents stay on the chassis and basket mass stays on the chassis inertial. Potential shaping uses a graph-safe coefficient of 5 instead of 2. Evaluation still has `guidance_weight=0` and teacher mix 0; promotion gates do not change. Log `basket_distance_m` / `basket_xy_m` / `easy_far_frac` for deposit. Jaw-overload failure applies to grasp/detach, not to DEPOSIT_ONLY fruit already falling past the gripper. A successful easy deposit is not a field harvest. Spawn-in-volume and liner teleport stay disallowed; collision must still resist clipping through the crate during the carry.
