...
`--easy` is an experimental student-side shortcut on top of that chain, not a second curriculum. Deposit resets spawn the arm **outside** the crate (robot-side of the front wall, above the rim) with a **free** fruit at the TCP and the jaw closed. The start recedes toward the home-side carry over 400 PPO updates (`easy_far_frac` 0→1) so the policy has to move the fruit to the basket instead of dropping from inside the opening. The privileged teacher still aims at a high drop pose over the rim (`hover_clearance_m=0.28`) and opens the jaw when fruit XY is over the basket (`teacher_mix=0.4` unless overridden). Potential shaping uses a graph-safe coefficient of 5 instead of 2. Evaluation still has `guidance_weight=0` and teacher mix 0; promotion gates do not change. Log `basket_distance_m` / `basket_xy_m` / `easy_far_frac` for deposit. Jaw-overload failure applies to grasp/detach, not to DEPOSIT_ONLY fruit already falling past the gripper. A successful easy deposit is not a field harvest. Spawn-in-volume and liner teleport stay disallowed; collision must still resist clipping through the crate during the carry.
