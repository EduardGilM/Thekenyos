...
`--easy` starts a **free** kiwi between the pads at a random IK-safe pose 32 cm outside the crate, 10 cm above the rim and 10–14 cm off the opening centerline (front-side of the hole, still inside the 15 cm open radius; 8 cm X span, 4 cm Y span, 8 cm Z span; 12 cm XY clips the wrist, 22 cm XY puts the pads through the front wall). A 16–26 cm Y start plus a backward swing tracked the exterior side wall. Then runs a **jaw-only script**: hold the tightest contacting close-fraction still under the 15 N fruit-contact gate, tighten if TCP-fruit slip exceeds 4 cm, and open once the fruit COM is over the basket **and below the rim**. The close-fraction sweep stays at 40 cm XY / 28 cm above the rim, centered, so a closer or side start cannot poison it. Easy basket shaping uses a 0.60 m length and a **+100 deposit** (engineering jackpot vs the default +20, which lost to −25 ground). 0.25 m shaping is flat at crate-scale 0.7–1.2 m carries. XY-only release at hover height dumps beside the liner. A light static keeper (close 0.25–0.30) dumps as soon as the arm moves. Open/closed come from pad-gap (Spot `arm_f1x` is closed near 0, open at −π/2), not from `jnt_range` order. The student arm is ordinary PPO. This is not a weld, not an arm teacher, and not a tissue-safe force. Eval still has `guidance_weight=0` and `teacher_mix=0`; the jaw script stays on because `--easy` is still enabled. Promotion gates do not change. `training_ready` stays false.
