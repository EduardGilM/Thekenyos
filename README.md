...
`--easy` starts a **free** kiwi between the pads at a random IK-safe pose outside the crate, then runs a **jaw-only script**: hold a close-fraction under the 15 N fruit-contact gate while the fruit is away from the opening, and open once fruit XY is over the basket. The student arm is ordinary PPO. This is not a weld, not an arm teacher, and not a tissue-safe force. Eval still has `guidance_weight=0` and `teacher_mix=0`; the jaw script stays on because `--easy` is still enabled. Promotion gates do not change. `training_ready` stays false.
