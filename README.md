...
`--easy` starts a **free** kiwi between the pads at a random IK-safe pose 22 cm outside the crate (12 cm X span; 12 cm still clips the wrist), then runs a **jaw-only script**: hold the tightest contacting close-fraction still under the 15 N fruit-contact gate, tighten if TCP-fruit slip exceeds 4 cm, and open once the fruit COM is over the basket **and below the rim**. XY-only release at hover height dumps beside the liner. A light static keeper (close 0.25–0.30) dumps as soon as the arm moves. Open/closed come from pad-gap (Spot `arm_f1x` is closed near 0, open at −π/2), not from `jnt_range` order. The student arm is ordinary PPO. This is not a weld, not an arm teacher, and not a tissue-safe force. Eval still has `guidance_weight=0` and `teacher_mix=0`; the jaw script stays on because `--easy` is still enabled. Promotion gates do not change. `training_ready` stays false.
