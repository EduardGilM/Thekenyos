...
`--easy` is an experimental student-side shortcut on top of that chain, not a second curriculum. Deposit resets sample **random** IK-accepted TCP poses outside the crate (40 cm robot-side margin, 28 cm rim clearance, bounded x/y/z jitter) so the wrist is not spawned through the liner, then place a **free** fruit at the pad-collision pocket (the nearest jaw/finger collision geoms; Spot's `hand_tcp` already sits between those pads, so this is not a pull toward the knuckle). That pocket is stored in the TCP body frame (~15 cm from the knuckle origin); spawn keeps it when it is within 8 cm of the TCP site and only falls back to the TCP if the sweep returns a knuckle-origin coordinate. Pad centres that sit in front of the teeth are projected onto the TCP axis (max 5 cm behind the site). A 2 cm mouth inset overlapped the palm (easy11 sweep 423 N / 0.75 m slip), so spawn uses the TCP opening; fruit is placed with the jaw open, then the close-fraction sweep shuts around that same point. This is not a 12 cm pull to the knuckle. A one-time CPU sweep of eight jaw close-fractions (0.40–1.00) commands the jaw actuator (not a qpos slam) and ignores the first contact frames so spawn overlap is not scored as the hold; it picks a retaining command under the 15 N fruit-contact load gate — one step tighter than the lightest keeper when several work, and the tightest close still under 15 N when nothing retains. The privileged teacher forces that hold while carrying to a high drop pose over the rim, then opens when fruit XY is over the basket (`teacher_mix` stays at 1.0 until an update records at least 8 deposits, then linearly drops to 0 over 60 PPO updates; the teacher assignment is per-world for the PPO chunk, not a per-step coin flip). Contact force is the result of the jaw position command, not a measured tissue-safe force. No weld, no liner teleport, no spawn inside the basket volume. Fast-scene load moves `basket_floor` / `basket_liner` onto a massless `basket_shell` child of the chassis and adds explicit arm-crate contact pairs so Spot's chassis–arm excludes and `filterparent` no longer let the gripper occupy the crate; visual vents stay on the chassis and basket mass stays on the chassis inertial. Potential shaping uses a graph-safe coefficient of 5 instead of 2. One-world nonfinite/overflow faults reset that world instead of killing the 3072-world job; `latest.pt` is overwritten every update so a crash does not throw away the previous weights. Evaluation still has `guidance_weight=0` and teacher mix 0; promotion gates do not change. Log `basket_distance_m` / `basket_xy_m` / `easy_hold_close_mean` / `hand_load_*_N` / `teacher_mix` for deposit. Jaw-overload failure applies to grasp/detach, not to DEPOSIT_ONLY fruit already falling past the gripper. A successful easy deposit is not a field harvest. Spawn-in-volume and liner teleport stay disallowed.
